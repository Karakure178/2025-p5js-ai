...
let rotationSpeeds = [];
const colors = [];
//const colorSelect = ["#0F828C", "#065084", "#320A6B", "#FFDE63"];
//const colorSelect = ["#799EFF", "#FEFFC4", "#FFDE63", "#FFBC4C"];
const colorSelect = ["#34699A", "#FFBC4C", "#FF894F"];
...
function draw() {
  ...
  for (let i = 0; i < numShapes; i++) {
    for (let j = 0; j < numShapes; j++) {
      ...
      // より有機的な波動（手描き風の不規則性）
      let wave1 = sin(d * 0.04 + times * 1.5 + noise(i * 0.1, j * 0.1) * 2);
      let wave2 = cos(d * 0.06 + times * -1.2 + noise(i * 0.2, j * 0.2) * 1.5);
      let wave3 = sin(normalizedD * PI * 1.8 + times * 2.2);
      ...
      // 手描き風の不規則な位置調整
      let jitterX = noise(i * 0.5, j * 0.5, times * 0.5) * 3 - 1.5;
      let jitterY = noise(i * 0.7, j * 0.7, times * 0.3) * 3 - 1.5;

      push();
      translate(x + jitterX, y + jitterY, sin(d * 0.03 + times) * 15);
      ...
      // よりシンプルな形状（スケッチ風）
      let shapeType = floor(map(sin(d * 0.02 + times * 0.6), -1, 1, 0, 2));
  ...
  const shaderImage = () => {
    ...
    theShader1.setUniform(`u_times`, frameCount / 30);
    filter(theShader1);
  };
