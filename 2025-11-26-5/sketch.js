...
function setup() {
  ...
  for (let curveIndex = 0; curveIndex < numCurves; curveIndex++) {
    ...
    for (let i = 0; i < numCircles; i++) {
      ...
      // 0.5を中心として、端に行くほど小さくなる (山型の分布)
      let sizeScale = 1.0 - abs(t - 0.5) * 2; // 0.5の時に1.0、0と1の時に0になる
      let baseSize = map(sizeScale, 0, 1, 5, 20); // 基本サイズを5〜20pxに
      let randomOffset = random(-3, 3); // ランダム性を維持

      circles.push({
        x: x,
        y: y,
        t: t,
        size: baseSize + randomOffset, // 基本サイズ + ランダム性
