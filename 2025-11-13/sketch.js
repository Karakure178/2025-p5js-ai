// 筆で描いたような四角形をクラス化して量産

let brushRects = []; // 四角形の配列
const colors = ["#BF1A1A", "#FF6C0C", "#060771", "#FFE08F"];

// =====================
// BrushRect クラス定義
// =====================
class BrushRect {
  constructor(x, y, w, h, rotation = 0, colorHex = "#060771") {
    this.x = x; // X座標
    this.y = y; // Y座標
    this.w = w; // 幅
    this.h = h; // 高さ
    this.rotation = rotation; // 回転角度（ラジアン）
    this.color = colorHex; // カラー（16進数）
  }

  // 四角形を描画
  display() {
    push();
    translate(this.x, this.y);
    rotate(this.rotation);

    // 筆のにじみと掠れを表現
    noFill();
    strokeWeight(8);

    // 四辺を個別に描画して筆のタッチを表現
    const corners = [
      [-this.w / 2, -this.h / 2], // 左上
      [this.w / 2, -this.h / 2], // 右上
      [this.w / 2, this.h / 2], // 右下
      [-this.w / 2, this.h / 2], // 左下
    ];

    // 各辺を筆のタッチで描く
    for (let i = 0; i < 4; i++) {
      const start = corners[i];
      const end = corners[(i + 1) % 4];
      this.drawBrushStroke(start[0], start[1], end[0], end[1]);
    }

    pop();
  }

  // 筆タッチの線を描画（掠れや揺らぎを表現）
  drawBrushStroke(x1, y1, x2, y2) {
    // 複数の薄い線を重ねて筆の質感を出す
    const layers = 15; // レイヤー数

    for (let layer = 0; layer < layers; layer++) {
      beginShape();

      // 透明度と太さを変えて重ねる（カラフルに）
      const alpha = map(layer, 0, layers - 1, 20, 80);
      const c = color(this.color);
      c.setAlpha(alpha);
      stroke(c);

      const segments = 50; // 線の分割数
      for (let i = 0; i <= segments; i++) {
        const t = i / segments;
        const x = lerp(x1, x2, t);
        const y = lerp(y1, y2, t);

        // ランダムな揺らぎで筆の掠れを表現
        const offsetX = randomGaussian(0, 3 - layer * 0.15);
        const offsetY = randomGaussian(0, 3 - layer * 0.15);

        // 筆圧の変化を模倣（両端が薄い）
        const pressure = sin(t * PI);
        const finalOffsetX = offsetX * pressure;
        const finalOffsetY = offsetY * pressure;

        vertex(x + finalOffsetX, y + finalOffsetY);
      }

      endShape();
    }
  }

  // ランダム生成ヘルパー
  static random() {
    const x = random(150, width - 150);
    const y = random(150, height - 150);
    const w = random(150, 350);
    const h = random(150, 350);
    const rotation = random(-PI / 12, PI / 12); // ±15度
    return new BrushRect(x, y, w, h, rotation);
  }
}

function setup() {
  const c = createCanvas(800, 800);
  c.parent("canvas-container");

  // getRandomCircles を使って四角形をランダムに配置
  generateRandomRects();

  noLoop(); // 静止画
}

keyPressed = () => {
  if (key === "s") {
    saveCanvas(canvas, "canvas", "png");
    //saveGif('canvas', 2);
  }
};

// getRandomCircles を使って四角形をランダムに配置
function generateRandomRects() {
  brushRects = [];

  // 重ならないようにランダムな位置を生成
  const positions = getRandomCircles(window, 30, width * 0.4, height * 0.4);

  for (const pos of positions) {
    const w = pos.z * 1.2; // z値をサイズに変換
    const h = pos.z * 1.2;
    const rotation = random(-PI / 12, PI / 12);

    // colors配列からランダムに色を選択
    const rectColor = random(colors);

    // 画面中央を基準に配置
    const x = width / 2 + pos.x;
    const y = height / 2 + pos.y;

    brushRects.push(new BrushRect(x, y, w, h, rotation, rectColor));
  }
}

function draw() {
  background(245, 242, 235); // 和紙のような背景色

  // 全ての四角形を描画
  for (const rect of brushRects) {
    rect.display();
  }
}

const getRandomCircles = (p, _num, _w, _h) => {
  let circles = [];
  for (let i = 0; i < _num; i++) {
    let x = p.random(-1, 1) * _w;
    let y = p.random(-1, 1) * _h;
    let z = p.random(30, 150); // z軸の値を円の大きさとして使用
    if (circles.every((c) => p.dist(x, y, c.x, c.y) > (z + c.z) * 0.45)) {
      circles.push(p.createVector(x, y, z));
    }
  }
  return circles;
};
