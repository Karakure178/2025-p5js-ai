...
`,
  fs: `
   ...
   void main() {
      ...
      // 複数の波動効果を合成
      float wave1 = sin(dist * 20.0 + u_time * 2.0) * 0.02;
      float wave2 = cos(uv.x * 15.0 + u_time * 1.5) * 0.015;
      float wave3 = sin(uv.y * 12.0 + u_time * -1.8) * 0.015;
      
      // ノイズによる歪み
      vec2 noisePos = uv * 3.0 + u_time * 0.1;
      float noiseX = fbm(noisePos) * 0.03;
      float noiseY = fbm(noisePos + vec2(100.0, 100.0)) * 0.03;
      
      // 螺旋効果
      float angle = atan(uv.y - 0.5, uv.x - 0.5);
      float spiral = sin(angle * 8.0 + dist * 25.0 + u_time * 3.0) * 0.01;
      
      // 渦効果
      float vortex = u_time * 0.5 + dist * 10.0;
      uv.x += cos(vortex) * spiral;
      uv.y += sin(vortex) * spiral;
      
      // 波動の合成適用
      uv.x += wave1 + wave2 + noiseX;
      uv.y += wave1 + wave3 + noiseY;
      ...
      // グリッチ効果（ランダムに発生）
      float glitch = step(0.98, noise(vec2(floor(originalUV.y * 20.0), u_time * 10.0)));
      if (glitch > 0.5) {
          uv.x += sin(u_time * 50.0) * 0.1;
          tex.rgb = mix(tex.rgb, vec3(1.0, 0.0, 1.0), 0.5);
      }
