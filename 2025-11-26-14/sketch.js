...
function fillCircleWithTriangles(centerX, centerY, radius) {
  ...
  for (let tri of triangles) {
    // 塗りつぶし
    fill(tri.hue, tri.saturation, tri.brightness, 60);
    noStroke();
    triangle(
      tri.vertices[0].x,
      tri.vertices[0].y,
    ...
    // エッジを多重描画で手書き風に（もっと大きくずらす）
    stroke(tri.hue, tri.saturation, tri.brightness - 20, 50);
    strokeWeight(2);
    noFill();

    // 複数回描画して手書き感を出す
    for (let k = 0; k < 6; k++) {
      let jitter = 2.5; // ジッター量を増やす

      beginShape();
      for (let v of tri.vertices) {
        let offsetX = random(-jitter, jitter);
        let offsetY = random(-jitter, jitter);
